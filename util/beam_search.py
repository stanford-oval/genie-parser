...
def sparse_boolean_mask(tensor, mask):
    ...
    return tf.SparseTensor(
        ...
        dense_shape=tf.cast(tf.stack([mask_shape[0], tf.reduce_max(mask_lens)]), tf.int64) # For 2D only
    )
...
class BeamSearchHelper(object):
    ...
    def __init__(self, cell, beam_size, stop_token, initial_state, initial_input,
        ...
        self.scope = scope
        self.initial_context_state = initial_context_state
        if self.initial_context_state is None:
            self.initial_context_state = tf.ones((1,))
