...
public class ThingTalkFeatureComputer implements FeatureComputer {
  ...
  private String tokenAfter(Example ex, Derivation deriv) {
    ...
    if (deriv.end == n - 1)
      return null;
    if (ex.token(deriv.end + 1).equals("''"))
      return deriv.end < n - 2 ? ex.token(deriv.end + 2) : null;
    else
      return ex.token(deriv.end + 1);
  }

  private void extractAnchorBoundaries(Example ex, Derivation deriv) {
    if (deriv.rule.isAnchored()) {
      deriv.addFeature("anchorBoundary", "left=" + tokenBefore(ex, deriv));
      deriv.addFeature("anchorBoundary", "right=" + tokenAfter(ex, deriv));
    }
  }
