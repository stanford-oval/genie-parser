...
public class LocationLexicon {
	...
	private static final String URL_TEMPLATE = "http://nominatim.openstreetmap.org/search/?format=jsonv2&accept-language=en&limit=5&q=%s";
	private static LocationLexicon instance;

	private final GenericObjectCache<String, Collection<Entry>> cache = new GenericObjectCache<>(256);

	private LocationLexicon() {

	}

	public static LocationLexicon get() {
		if (instance == null)
			instance = new LocationLexicon();
		return instance;
	}
	...
	private Collection<Entry> lookupCacheMiss(String rawPhrase) {
		try {
			URL url = new URL(String.format(URL_TEMPLATE, URLEncoder.encode(rawPhrase, "utf-8")));
			if (opts.verbose >= 3)
