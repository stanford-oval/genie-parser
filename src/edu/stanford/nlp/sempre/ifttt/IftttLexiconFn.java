 ...
 */
public class IftttLexiconFn extends SemanticFn {
  ...
  private IftttLexicon lexicon;

  private IftttLexicon.Mode mode;
  private String filter;
  private boolean floating = false;

  public IftttLexiconFn() {
  }
  ...
  public void init(LispTree tree) {
    ...
    lexicon = IftttLexicon.getForLanguage(languageTag);

    String mode = tree.child(2).value;
    // mode
    try {
      this.mode = IftttLexicon.Mode.valueOf(mode.toUpperCase());
  ...
  public DerivationStream call(Example ex, Callable c) {
    ...
    if (!floating)
      ...
      phrase = null;
    Iterator<IftttLexicon.Entry> entries;
    try {
  ...
  public class ThingpediaDerivationStream extends MultipleDerivationStream {
    ...
    private Iterator<IftttLexicon.Entry> entries;
    private String phrase;

    public ThingpediaDerivationStream(Example ex, Callable c, Iterator<IftttLexicon.Entry> entries,
        String phrase) {
      this.ex = ex;
      this.callable = c;
    ...
    public Derivation createDerivation() {
      ...
      IftttLexicon.Entry entry = entries.next();
