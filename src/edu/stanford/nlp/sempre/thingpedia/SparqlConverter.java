...
public class SparqlConverter {
	...
	public static String executeAndWrap(Formula formula) {
		...
		return jf.toLispTree().toString();
	}

	// work around JavaExecutor annoyances
	public static String executeAndWrap(Value value) {
		return executeAndWrap(new ValueFormula<>(value));
	}

	public static String executeAndWrap(String value) {
		return executeAndWrap(new ValueFormula<>(new StringValue(value)));
	}

	public SparqlConverter(Formula rootFormula) throws BadFormulaException {
		Ref<PrimitiveFormula> head = new Ref<PrimitiveFormula>();
		SparqlConverter.Box box = new Box();
	...
	private Formula stripOuterLambdas(SparqlConverter.Box box, Formula formula) {
	...
	private SparqlSelect closeExistentialScope(SparqlBlock block, Ref<PrimitiveFormula> head, SparqlConverter.Box box) {
	...
	private void addSelectVar(SparqlConverter.Box box, SparqlSelect select, SparqlBlock block, VariableFormula var,
			boolean isAuxiliary) {
	...
	private VariableFormula ensureIsVar(SparqlConverter.Box box, SparqlBlock block, Ref<PrimitiveFormula> head) {
	...
	private void updateAsValues(SparqlConverter.Box box, VariableFormula var, String asValue) {
	...
	private SparqlBlock convert(Formula rawFormula, Ref<PrimitiveFormula> head, Ref<PrimitiveFormula> modifier,
		...
		if (rawFormula instanceof ValueFormula) { // e.g.,
			...
			if (modifier.value == null)
				modifier.value = newVar();
			SparqlConverter.Box newBox = createNewBox(formula.body, box); // Create new
			// environment
			...
			if (useOrderBy) {
				...
				// Recurse
				SparqlConverter.Box newBox = createNewBox(formula.head, box); // Create new
				// environment
			...
			// Recurse
			SparqlConverter.Box newBox = createNewBox(formula.child, box);
	...
	private SparqlConverter.Box createNewBox(Formula formula, SparqlConverter.Box box) {
		SparqlConverter.Box newBox = new Box();
	...
	// constraints.
	private void returnAsValuesMap(SparqlConverter.Box box, SparqlConverter.Box newBox) {
