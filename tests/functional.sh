...
for problem in semparse_thingtalk semparse_thingtalk_noquote  ; do
    ...
    for model in luinet_copy_seq2seq ; do
        for grammar in bottomup ; do
            for options in ""  ; do
                pipenv run $SRCDIR/../luinet-trainer --problem $problem --data_dir $workdir --output_dir $workdir/model.$i --model $model --hparams_set ${model_hparams[$model]} --hparams "grammar_direction=$grammar,$options" --save_checkpoints_secs 10 --eval_throttle_seconds 100 --train_steps 2 --curriculum
