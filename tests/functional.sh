        ...
        for grammar in bottomup topdown linear ; do
            for hparams in transformer_tiny_luinet ; do
                for options in "pointer_layer=attentive" "pointer_layer=decaying_attentive" ; do
                    ...
                    # beam search decode
                    pipenv run $SRCDIR/../luinet-decoder --problem $problem --data_dir $workdir --output_dir $workdir/model.$i --model $model --hparams_set $hparams --hparams "grammar_direction=$grammar,$options" --decode_hparams 'beam_size=4,alpha=0.6'
