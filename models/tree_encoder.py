...
class TreeEncoder(BaseEncoder):
    ...
    def encode(self, inputs: tf.Tensor, input_length: tf.Tensor, parses : tf.Tensor):
        with tf.variable_scope('treeenc'):
            ...
            LoopState=namedtuple('TreeEncoderLoopState', ['finished', 'time', 'buffer_ptr',
                'stack_hta', 'stack_cta', 'back_pointer_ta', 'rnn_state'])
            ...
            def body(finished, time, buffer_ptr, stack_hta, stack_cta, back_pointer_ta, rnn_state):
                ...
                buffer_top_indices = tf.stack((buffer_ptr, batch_indices), axis=1)
                buffer_top = tf.gather_nd(buffer, buffer_top_indices)
                print('buffer_top', buffer_top)
                # stack_htop : the h vector at the top of the stack
                stack_htop = stack_hta.read(stack_top)
                print('stack_htop', stack_htop)
                # ptr_stack_prev_to_top : the pointer (index) to the second element from the top
                # in the stack
                ptr_stack_prev_to_top = back_pointer_ta.read(stack_top)
                print('ptr_stack_prev_to_top', ptr_stack_prev_to_top)
                ...
                stack_h_prev_to_top = stack_hta.gather(ptr_stack_prev_to_top)
                diag_indices = tf.stack((batch_indices, batch_indices), axis=1)
                stack_h_prev_to_top = tf.gather_nd(stack_h_prev_to_top, diag_indices)
                print('stack_h_prev_to_top', stack_h_prev_to_top)
                
                # apply the rnn
                rnn_inputs = tf.concat((buffer_top, stack_htop, stack_h_prev_to_top), axis=1)
                ...
                if self._train_syntactic_parser:
                    with tf.variable_scope('parser'):
                        ...
                        next_op_is_reduce = tf.where(buffer_ptr >= input_length, True, next_op_is_reduce)
                else:
                ...
                print('if_shift_stack_ctop', if_shift_stack_ctop)
                if_shift_stack_htop = buffer_top[:, self._output_size:]
                print('if_shift_stack_htop', if_shift_stack_htop)
                ...
                if_reduce_back_pointer_top = tf.gather_nd(if_reduce_back_pointer_top, diag_indices)
                print('if_reduce_back_pointer_top', if_reduce_back_pointer_top)
                if_reduce_buffer_ptr = buffer_ptr
                
                new_stack_htop = tf.where(next_op_is_reduce, if_reduce_stack_htop, if_shift_stack_htop) 
                new_stack_ctop = tf.where(next_op_is_reduce, if_reduce_stack_ctop, if_shift_stack_ctop)
                new_back_pointer_top = tf.where(next_op_is_reduce, if_reduce_back_pointer_top, if_shift_back_pointer_top)
                new_buffer_ptr = tf.where(next_op_is_reduce, if_reduce_buffer_ptr, if_shift_buffer_ptr)
                ...
                new_buffer_ptr = tf.where(finished, buffer_ptr, new_buffer_ptr)
                new_stack_htop = tf.where(finished, stack_htop, new_stack_htop)
                new_stack_hta = stack_hta.write(stack_top+1, new_stack_htop)
                new_stack_ctop = tf.where(finished, stack_ctop, new_stack_ctop)
                new_stack_cta = stack_cta.write(stack_top+1, new_stack_ctop)
                new_back_pointer_top = tf.where(finished, ptr_stack_prev_to_top, new_back_pointer_top)
                new_back_pointer_ta = back_pointer_ta.write(stack_top+1, new_back_pointer_top)
                
                new_time = time + 1
                
                # collect everything and make a new LoopState
                return LoopState(finished, new_time, new_buffer_ptr, new_stack_hta, new_stack_cta,
                                 new_back_pointer_ta, next_rnn_state)
            ...
            initial_back_pointer_ta = initial_back_pointer_ta.write(0, tf.zeros((batch_size,), dtype=tf.int32)).write(1, tf.ones((batch_size,), dtype=tf.int32))
            
            initial_rnn_state = rnn_cell.zero_state(batch_size, dtype=tf.float32)
            initial_finished = tf.zeros((batch_size,), dtype=tf.bool)
            initial_time = 0
            initial_buffer_ptr = tf.zeros((batch_size,), dtype=tf.int32)
            
            initial_loop_state = LoopState(initial_finished, initial_time, initial_buffer_ptr,
                                           initial_stack_hta, initial_stack_cta, initial_back_pointer_ta, initial_rnn_state)
            _, final_time, _, final_stack_hta, final_stack_cta, final_back_pointer_ta, _ = tf.while_loop(cond, body, initial_loop_state, swap_memory=True)
            
            final_stack_htop = final_stack_hta.read(final_time+1)
            print('final_stack_htop', final_stack_htop)
            final_stack_ctop = final_stack_cta.read(final_time+1)
            print('final_stack_ctop', final_stack_ctop)
            
            # use the final backpointer TA, or bad things happen
            _ = final_back_pointer_ta.stack()
