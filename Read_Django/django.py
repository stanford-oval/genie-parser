...
class DjangoGrammar(ShiftReduceGrammar):
    ...
    def __init__(self, filename=None, **kw):
                ...
                for token in line.strip().split(' '):
                    if token:
                        if (token[0].isalpha() or token.startswith('_')) and token not in kwlist2:
                            if token.find('$') == -1 and token.find('STR') == -1 and token.find('NUM') == -1:
                                idents.add(token)
        ...
        strings = ["'_STR:" + str(i) + "_'" for i in range(201)]
        strings.extend(["r'_STR:" + str(i) + "_'" for i in range(201)])
        strings.extend(['', "", ' ', " "])
        numbers = ["'_NUM:" + str(i) + "_'" for i in range(11)]
